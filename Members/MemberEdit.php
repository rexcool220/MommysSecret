...
header("Content-Type:text/html; charset=utf-8");
	...
	$type = $dataArray[10];
 	...
 	$sql = "INSERT INTO `Members` (`姓名`, `FB帳號`, `手機號碼`, `郵遞區號＋地址`,`全家店到店服務代號` ,`寄送方式`, `運費`, `備註`, `Type`, `FBID`)
 	VALUES (\"$name\", \"$fbAccount\", \"$phoneNumber\", \"$address\", \"$familyNumber\", \"$shippingWay\", \"$shippingFee\", \"$comment\", \"$type\", \"$fbID\")
 	ON DUPLICATE KEY UPDATE `姓名`='$name', `FB帳號`='$fbAccount', `手機號碼`='$phoneNumber', `郵遞區號＋地址`='$address',`全家店到店服務代號`='$familyNumber' ,`寄送方式`='$shippingWay', `運費`='$shippingFee', `備註`='$comment', `Type`='$type'";
