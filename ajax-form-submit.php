...
if(!$accessToken)
{
	...
	for($i = 0;$i < $itemSpecCounts;$i++)
	{
		$sql = "INSERT INTO `ItemCategory`(`ItemID`, `品項`, `規格`, `單價`, `成本`, `批發價`, `月份`, `收單日期`, `廠商`) VALUES ('$itemID', '$itemName', '$itemSpec[$i]', '$itemPrice[$i]', '$itemCost[$i]', '$itemShopPrice[$i]', '$month', '$closeDate', '$vendor')
		ON DUPLICATE KEY UPDATE `品項`=\"$itemName\", `單價`=\"$itemPrice[$i]\", `成本`=\"$itemCost[$i]\", `批發價`=\"$itemShopPrice[$i]\", `月份`=\"$month\", `收單日期`=\"$closeDate\", `廠商`=\"$vendor\"";
